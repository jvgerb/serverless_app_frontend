<template>
  <ListPageWrap :headers="headers" :items="items" :loading="loading">
    <template v-slot:filter-template>
      ...
      <div class="form-field">
        <select v-model="remoteControl" @change="update">
          <option value>Remote Control</option>
          <option v-for="option in remoteControls" :value="option" :key="option">{{ option }}</option>
        </select>
        <div class="arrow-divider"></div>
      </div>
    </template>
    <template v-slot:table-template="slotScope">
      ...
      <td>{{ slotScope.props.item['interfaceProtocol'] }}</td>
      ...
      <td>{{ slotScope.props.item['operationMode'] }}</td>
...
export default class extends Vue {
  ...
  headers = [
    ...
    { text: 'Interface Protocol', value: 'interfaceProtocol' },
    { text: 'Serial Number', value: 'serialNumber' },
    { text: 'Purchase Date', value: 'purchaseDate' },
    { text: 'Warranty Until', value: 'warrantyUntil' },
    { text: 'Operation Mode', value: 'operationMode' },
  ...
  async update() {
    this.loading = true;
    this.items = await api.pod.getControlUnitList(
      this.manufacturer,
  ...
  async created() {
    ...
    this.remoteControls = [
      ...new Set(this.items.map((x: any) => x.data.control_unit_info.remote_control)),
    ];
  }
}
</script>
