<template>
  <ListPageWrap :headers="headers" :items="items" :loading="loading">
    <template v-slot:filter-template>
      ...
      <div class="form-field">
        <select v-model="storageType" @change="update">
          <option value>Storage Type</option>
          <option v-for="option in storageTypes" :value="option" :key="option">{{ option }}</option>
        </select>
        <div class="arrow-divider"></div>
      </div>
    </template>
    <template v-slot:table-template="slotScope">
      ...
      <td>{{ slotScope.props.item['storageType'] }}</td>
...
export default class extends Vue {
  ...
  async update() {
    this.loading = true;
    this.items = await api.pod.getWaterStorageList(
      this.manufacturer,
  ...
  async created() {
    ...
    this.storageTypes = [
      ...new Set(this.items.map((x: any) => x.data.water_storage_info.storage_type)),
    ];
  }
}
</script>
