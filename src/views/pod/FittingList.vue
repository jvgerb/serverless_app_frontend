<template>
  <ListPageWrap :headers="headers" :items="items" :loading="loading">
    <template v-slot:filter-template>
      ...
      <div class="form-field">
        <select v-model="fittingType" @change="update">
          <option value>Fitting Type</option>
          <option v-for="option in fittingTypes" :value="option" :key="option">{{ option }}</option>
        </select>
        <div class="arrow-divider"></div>
      </div>
    </template>
    <template v-slot:table-template="slotScope">
      ...
      <td>{{ slotScope.props.item['fittingType'] }}</td>
...
export default class extends Vue {
  ...
  headers = [
    ...
    { text: 'Fitting Type', value: 'fittingType' },
  ...
  async update() {
    this.loading = true;
    this.items = await api.pod.getFittingList(
      this.manufacturer,
  ...
  async created() {
    ...
    this.fittingTypes = [
      ...new Set(this.items.map((x: any) => x.data.fitting_info.fitting_type)),
    ];
  }
}
</script>
