<template>
  <ListPageWrap :headers="headers" :items="items" :loading="loading">
    <template v-slot:filter-template>
      ...
      <div class="form-field">
        <select v-model="fuelTankCategory" @change="update">
          <option value>Fuel Tank Category</option>
          <option v-for="option in fuelTankCategories" :value="option" :key="option">{{ option }}</option>
        </select>
        <div class="arrow-divider"></div>
      </div>
    </template>
    <template v-slot:table-template="slotScope">
      ...
      <td>{{ slotScope.props.item['totalVolume'] }}</td>
...
export default class extends Vue {
  ...
  async update() {
    this.loading = true;
    this.items = await api.pod.getFuelTankList(
      this.manufacturer,
      this.designationType,
      this.fuelTankCategory
    );
    this.loading = false;
  }
  ...
  async created() {
    ...
    this.fuelTankCategories = [
      ...new Set(this.items.map((x: any) => x.data.fuel_tank_info.fuel_category)),
    ];
  }
}
</script>
